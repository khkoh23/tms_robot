...
MoveArmNamedTargetNode::MoveArmNamedTargetNode(
...
BT::PortsList MoveArmNamedTargetNode::providedPorts()
{
  return {
    ...
    BT::InputPort<std::string>("planning_group", "ur_arm")
  };
}
...
BT::NodeStatus MoveArmNamedTargetNode::onStart()
{
  if (is_cancel_requested_from_blackboard(config())) {
    RCLCPP_WARN(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Cancel requested before motion start");
  ...
  if (!target) {
    RCLCPP_ERROR(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Missing required input port: target");
    return BT::NodeStatus::FAILURE;
  }
  ...
  if (!planning_group) {
    RCLCPP_ERROR(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Missing required input port: planning_group");
    return BT::NodeStatus::FAILURE;
  }
  ...
  try {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Planning move to named target '%s' using planning group '%s'",
      target_name_.c_str(),
    ...
    if (plan_result.val != moveit_msgs::msg::MoveItErrorCodes::SUCCESS) {
      RCLCPP_ERROR(
        rclcpp::get_logger("MoveArmNamedTargetNode"),
        "Planning failed for target '%s'. MoveIt error code: %d",
        target_name_.c_str(),
        plan_result.val);
      return BT::NodeStatus::FAILURE;
    }
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Planning succeeded. Starting execution.");
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Exception during planning/execution start: %s",
      e.what());
    return BT::NodeStatus::FAILURE;
  }
}
...
BT::NodeStatus MoveArmNamedTargetNode::onRunning()
{
  if (is_cancel_requested_from_blackboard(config())) {
    RCLCPP_WARN(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Cancel requested. Stopping MoveIt execution.");
  ...
  try {
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "MoveIt execution failed. Error code: %d",
    ...
    RCLCPP_ERROR(
      rclcpp::get_logger("MoveArmNamedTargetNode"),
      "Exception while getting execution result: %s",
      e.what());
    return BT::NodeStatus::FAILURE;
  }
}
...
void MoveArmNamedTargetNode::onHalted()
{
  RCLCPP_WARN(
    rclcpp::get_logger("MoveArmNamedTargetNode"),
    "BT node halted. Stopping MoveIt execution.");
